...
public class TargetSpawnerScript : MonoBehaviour
{
    ...
    public void Spawn()
    {
        for (int i = 0; i < attemptsToSpawn; i++)
        {
            ...
            float targetY = 5;
            float targetZ = 0.55f;
            Vector3 targetPos = new Vector3(targetX, targetY, targetZ - transform.position.z);
            ...
            Debug.Log("spawnable collisions length: " + spawnableCollisions[0].gameObject.layer);
            Debug.Log("colliders: ");
